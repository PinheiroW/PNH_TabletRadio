class ConfigSettingsClient
{
  ...
  void ConfigSettingsClient()
  {
	...
	void TransformToSendableConfig(ref DiPSettings m_DiPSettings)
	{
		...
		for(int o=0;o< m_DiPSettings.Tab1_Info.Count();o++)
		{
			Tab1_Info.Insert(m_DiPSettings.Tab1_Info.Get(o));
		}

		for(int f=0;f< m_DiPSettings.Tab2_Info.Count();f++)
		{
			Tab2_Info.Insert(m_DiPSettings.Tab2_Info.Get(f));
		}

		for(int l=0;l< m_DiPSettings.Tab3_Info.Count();l++)
		{
			Tab3_Info.Insert(m_DiPSettings.Tab3_Info.Get(l));
		}

		for(int b=0;b< m_DiPSettings.Tab4_Info.Count();b++)
		{
			Tab4_Info.Insert(m_DiPSettings.Tab4_Info.Get(b));
		}
	}
};

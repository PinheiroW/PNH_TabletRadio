...
class DiPJsonLoader<Class T>
{
	...
	static string ObjectToString(T data)
	{
		...
		if (!m_Serializer.WriteToString(data, true, string_data)) {
			// GetRspZLogger().LogError("ObjectToString | Could not stringify "+ data.ToString());
			return string.Empty;
		}
	...
	static void SaveToFile(string path, T data)
	{
		...
		if (!fh) {
			// GetRspZLogger().LogError("SaveToFile | File could not be created at "+ path);
			return;
		}
		...
		// GetRspZLogger().LogError("SaveToFile | Complete: " + path);
		CloseFile(fh);
	}
	...
	{
		if (!FileExist(path)) {
			// GetRspZLogger().LogError("LoadFromFile | File not found "+ path);
			return;
		}
		...
		if (!fh) {
			// GetRspZLogger().LogError("LoadFromFile | File could not be opened " + path);
			return;
		}
		...
		if (error != string.Empty || !success) {
			// GetRspZLogger().LogError("LoadFromFile | ERROR Parsing "+ path + error);
			return;
		}

		// GetRspZLogger().LogInfo("LoadFromFile | Loaded file: " + path);
		CloseFile(fh);
	}
};

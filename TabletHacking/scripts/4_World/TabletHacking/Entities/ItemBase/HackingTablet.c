class HackingTablet extends ItemBase{
	...
	void StartHackServer(ItemBase HackingTarget, PlayerBase Hacker){
		...
		if (hacker && hackingTarget){
			...
			if ( hackingTarget.GetHackID() != this.GetHackID() || hackingTarget.GetHackID() == 0){ //Starting A hack on a new object
				...
				m_HackTimeRemaining = GetTabletHackingConfig().GetHackTime(hackingTarget.GetType()) * 1000;
				...
				if (hacker.GetIdentity() && GetTabletHackingConfig().ScriptLogging){
					Print("[TabletHacking][Raid] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") has started hacking " + hackingTarget.GetType() + " with ID: " + HackID + " at " + hackingTarget.GetPosition());
				}
				GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") has started hacking " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition());
			} else {
				...
				resumed = " has resumed";
				if (hacker.GetIdentity() && GetTabletHackingConfig().ScriptLogging){
					Print("[TabletHacking][Raid] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") has resumed hacking " + hackingTarget.GetType() + " with ID: " + GetHackID() + " at " + hackingTarget.GetPosition());
				}
				GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") has resumed hacking " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition());
			}
	...
	bool CheckHackInterrupt(ItemBase HackingTarget, PlayerBase Hacker){
		...
		if (hacker && hacker.GetIdentity() && hackingTarget){
			HackableItem hackingData = GetTabletHackingConfig().GetHackableItem( hackingTarget.GetType() );
			#ifdef CodeLock
            ...
            if ( codelock && hackingData.Type != "") {
				if (!codelock.GetLockState()){
					...
					IsInterrupted = true;
					GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to codelock being unlocked");
				}
			} else { //Code Lock Removed
				IsInterrupted = true;
				GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to codelock missing");
			}
			#endif

			#ifdef RA_BASEBUILDING
            if (hackingData.Type != "") {
				if (!hackingTarget.IsCodeLocked()){
					//Code Lock not locked
					IsInterrupted = true;
					GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to codelock being unlocked");
				}
			...
			//Missing Batteries
			if (!GetTabletHackingConfig().CanHack( hackingTarget.GetType(), this.CountBatteries())){
				IsInterrupted = true;
				GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to Missing Batteries");
			}
			...
			if (DoInterrupt < InterruptChance){
				IsInterrupted = true;
				GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to interrupt Chance");
			}
			//Tablet is in hands
			if (hacker.GetItemInHands() != this && RequireInHands){
				IsInterrupted = true;
				GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to tablet dropped from hands");
			}
			//Hacker Left Radius
			if (vector.Distance(hackingTarget.GetPosition(), hacker.GetPosition()) > HackingRadius){
				IsInterrupted = true;
				GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to Hacker Leaving the hacking radius");
			}
			//Tablet is ruined
			if (this.IsRuined()){
				IsInterrupted = true;
				GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to Tablet Being Destroyed");
			}
			//Player has Disconnected
			if (hacker.IsPlayerDisconnected()){
				IsInterrupted = true;
				GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to Player Disconected");
			}
		} else {
			//This shouldn't happen, but Player and Target are inviald
			IsInterrupted = true;
			GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") had their hacking of " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition() + " Interrupted due to invalid Target or Player");
		}
	...
	void HackCompleted(ItemBase HackingTarget, PlayerBase Hacker){
		...
		#ifdef RA_BASEBUILDING
		RA_CodeLock codelock = hackingTarget.FindCodeLock();
		if(codelock)
		{
			codelock.SetState(CodeLockState.PENDING);
			codelock.ClearAuthorizations();
			hackingTarget.GetInventory().DropEntity(InventoryMode.SERVER, hackingTarget, codelock);
		...
		if (hackingData.Type != "") {
			...
			#endif
			if (hacker.GetIdentity() && GetTabletHackingConfig().ScriptLogging){
				Print("[TabletHacking][Raid] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") Hacked  " + hackingTarget.GetType() + " with ID: " + GetHackID() + " at " + hackingTarget.GetPosition());
			}
			GetGame().AdminLog("[TabletHacking][Raid][ID:" + GetHackID() + "] " + hacker.GetIdentity().GetName() + "(" +  hacker.GetIdentity().GetPlainId() + ") Hacked  " + hackingTarget.GetType() + " at " + hackingTarget.GetPosition());
	...
	void TurnOnTablet(){
		...
		if(!IsRuined()){
			SetObjectTexture(0, "ParagonStorage\\Tablet\\data\\Tablet_Black_on.paa");
			SetObjectTexture( GetHiddenSelectionIndex("Camo"), "ParagonStorage\\Tablet\\data\\Tablet_Black_on.paa");
		}
	}
	...
	void TurnOffTablet(){
		...
		if(!IsRuined()){
			SetObjectTexture(0, "ParagonStorage\\Tablet\\data\\Tablet_Black.paa");
			SetObjectTexture( GetHiddenSelectionIndex("Camo"), "ParagonStorage\\Tablet\\data\\Tablet_Black.paa");
		}
	}
	...
	int CountBatteries(){
		int count = 0;
		HackingBattery tempBattery1;
		HackingBattery tempBattery2;
		HackingBattery tempBattery3;
		HackingBattery tempBattery4;
		HackingBattery tempBattery5;
		
		if (Class.CastTo(tempBattery1, FindAttachmentBySlotName("Att_HackingBattery_1"))){
			if (!tempBattery1.IsRuined()){
				count++;
			}
		}
		if (Class.CastTo(tempBattery2, FindAttachmentBySlotName("Att_HackingBattery_2"))){
			if (!tempBattery2.IsRuined()){
				count++;
			}
		}
		if (Class.CastTo(tempBattery3, FindAttachmentBySlotName("Att_HackingBattery_3"))){
			if (!tempBattery3.IsRuined()){
				count++;
			}
		}
		if (Class.CastTo(tempBattery4, FindAttachmentBySlotName("Att_HackingBattery_4"))){
			if (!tempBattery4.IsRuined()){
				count++;
			}
		}
		if (Class.CastTo(tempBattery5, FindAttachmentBySlotName("Att_HackingBattery_5"))){
	...
	override void EEItemAttached(EntityAI item, string slot_name){
		...
		if (GetGame().IsServer() && (slot_name == "Att_HackingBattery_1" || slot_name == "Att_HackingBattery_2" || slot_name == "Att_HackingBattery_3" || slot_name == "Att_HackingBattery_4" || slot_name == "Att_HackingBattery_5")){
			m_TabletON = true;
			SetSynchDirty();
		}
	}
	...
	override void EEItemDetached(EntityAI item, string slot_name){
		...
		if (GetGame().IsServer() && (slot_name == "Att_HackingBattery_1" || slot_name == "Att_HackingBattery_2" || slot_name == "Att_HackingBattery_3"  || slot_name == "Att_HackingBattery_5"  || slot_name == "Att_HackingBattery_5")){
			if (CountBatteries() == 0){
				m_TabletON = false;
				SetSynchDirty();
			}
	...
	void SendPlayerMessage(PlayerBase hacker, string heading, string text){
		...
		string Message = text;
		PlayerBase Hacker = PlayerBase.Cast(hacker);
		string NotificationIcon = "TabletHacking/GUI/Images/hacking.paa";
		if (Hacker.GetIdentity() && GetTabletHackingConfig().Notification == 0){
		...
		if (Hacker.GetIdentity() && (GetTabletHackingConfig().Notification == 1 ||  GetTabletHackingConfig().Notification == 2)){
			GetRPCManager().SendRPC("HACK", "RPCTabletHackingNotifcation", new Param3< string, string, float >( Heading, Message, 8 ), true, Hacker.GetIdentity());
		}
	}
